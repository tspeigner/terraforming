output "public_dns" {
  value = "${aws_instance.cd4pe_client.public_dns}"
}

# Declare the instance resource here
resource "aws_instance" "cd4pe_client" {
  count                       = "${var.count}"
...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo bash -c \"curl -k https://ip-10-98-10-11.us-west-2.compute.internal:8140/packages/current/install.bash | sudo bash -s extension_requests:pp_role=cd4pe_client\"",
...
tags {
    Name    = "${format("cd4pe-client-%02d.inf.puppet.vm", count.index+1,)}"
    Owner   = "Tommy"
    Purpose = "TSE Test"
    Tech    = "Terraform"
  }
